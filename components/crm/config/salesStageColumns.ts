...
export const salesStageColumns: ColumnDef<any>[] = [
  ...
  {
    accessorKey: 'name',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: 'Stage Name' }),
  ...
  {
    accessorKey: 'color',
    header: ({ column }) => h(DataTableColumnHeader, { column, title: 'Color' }),
    cell: ({ row }) => h('span', { class: 'inline-block w-6 h-6 rounded-full border', style: `background:${row.getValue('color')}` }),
  },
  {
