...
const emit = defineEmits<{
  ...
  'autofill': [match: CrmCompanyLookupResult]
}>()

const { tenantId } = useTenant()
const open = ref(false)
const loading = ref(false)
const suggestions = ref<CrmCompanyLookupResult[]>([])
const lastAppliedKey = ref<string | null>(null)
const isSelecting = ref(false)

function getMatchKey(match: CrmCompanyLookupResult) {
  return match.id
}

function getDisplayLabel(match: CrmCompanyLookupResult) {
  const parts = [match.name]
  if (match.city)
    parts.push(match.city)
  if (match.website)
    parts.push(match.website)
  return parts.join(' · ')
}
...
async function fetchSuggestions(query: string) {
  ...
  loading.value = true
  try {
    const response = await $fetch<{ data: CrmCompanyLookupResult[] }>('/api/crm/company/lookup', {
      query: {
    ...
    const exactMatches = suggestions.value.filter(item => item.name.toLowerCase() === normalizedQuery)
...
function applySuggestion(match: CrmCompanyLookupResult, silent = false) {
