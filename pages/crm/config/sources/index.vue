...
// SSR-friendly fetch dos sources
const {
  data: sourcesRaw,
  pending,
  refresh: refreshSources,
} = await useAsyncData('crm-lead-sources', () => $fetch('/api/crm/lead_source'), { watch: [tenantId] })
...
async function handleDeleteConfirm() {
  ...
  await useFetch(`/api/crm/lead_source/${sourceToDelete.value.id}?tenant_id=${tenantId}`, { method: 'DELETE' })
...
async function handleMultiDeleteConfirm() {
  ...
  await Promise.all(idsToDelete.map(id => useFetch(`/api/crm/lead_source/${id}?tenant_id=${tenantId}`, { method: 'DELETE' })))
  selectedItems.value = []
  await refreshSources()
}
  ...
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground">Manage all lead sources for your CRM.</p>
      </div>
    ...
    <Dialog :open="showDialog" @update:open="showDialog = $event">
      <DialogContent class="max-w-lg w-full">
        <DialogHeader>
          <DialogTitle>{{ formMode === 'edit' ? 'Edit Source' : 'New Source' }}</DialogTitle>
        </DialogHeader>
        <SourceForm
          v-model="formModel"
          :mode="formMode"
    ...
    <div v-if="showDeleteDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="max-w-md w-full rounded-lg bg-white p-6 shadow-lg dark:bg-zinc-900">
        <h2 class="mb-2 text-lg font-bold">Delete Source</h2>
        <p class="mb-4">Are you sure you want to delete the source "{{ sourceToDelete?.name }}"? This action cannot be undone.</p>
    ...
    <div v-if="showMultiDeleteDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="max-w-md w-full rounded-lg bg-white p-6 shadow-lg dark:bg-zinc-900">
        <h2 class="mb-2 text-lg font-bold">Delete Multiple Sources</h2>
        <p class="mb-4">Are you sure you want to delete {{ selectedItems.length }} sources? This action cannot be undone.</p>
        <div class="flex justify-end gap-2">
