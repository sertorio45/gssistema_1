...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { error } = await client.from('crm_company').delete().eq('id', body.id).eq('tenant_id', tenantId)
    if (error) {
      throw createError({ statusCode: 500, message: error.message || 'Falha ao remover company' })
    }
