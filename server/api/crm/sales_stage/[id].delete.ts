...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { error } = await client.from('crm_sales_stage').delete().eq('id', id).eq('tenant_id', tenant_id)
    if (error) {
      throw createError({ statusCode: 500, message: error.message || 'Falha ao remover sales stage' })
    }
