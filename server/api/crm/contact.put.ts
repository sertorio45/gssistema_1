...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { data, error } = await client.from('crm_contact').update(updateData).eq('id', body.id).eq('tenant_id', tenantId).select().single()
    if (error) {
      throw createError({ statusCode: 500, message: error.message || 'Falha ao atualizar contact' })
    }
